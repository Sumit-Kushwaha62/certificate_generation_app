...
function App() {
  ...
  const shareWhatsApp = async () => {
    ...
    const a = document.createElement('a');
    a.href = dataURL; a.download = 'certificate.png'; a.click();
    setTimeout(() => window.open('https://wa.me/?text=Here%20is%20my%20certificate%20%F0%9F%8E%93', '_blank'), 800);
  };
  ...
  const shareEmail = async () => {
    ...
    const a = document.createElement('a');
    a.href = dataURL; a.download = 'certificate.png'; a.click();
    setTimeout(() => { window.location.href = 'mailto:?subject=My%20Certificate&body=Please%20find%20my%20certificate%20attached.'; }, 800);
  };

  const shareNative = async () => {
    setShareOpen(false);
    if (!navigator.share) { alert('Native share not supported in this browser.'); return; }
    try {
      const { canvas } = await getCanvasBlob();
      canvas.toBlob(async (blob) => {
        const file = new File([blob], 'certificate.png', { type: 'image/png' });
        await navigator.share({ title: 'My Certificate', files: [file] });
      }, 'image/png');
            ...
            {shareOpen && (
              <div className="absolute right-0 top-10 w-52 bg-white rounded-2xl shadow-xl border border-[#E8E8E8] z-50 overflow-hidden">
                <div className="px-4 py-3 border-b border-[#F0F0F0]">
                  ...
                  <p className="text-[10px] text-[#AAA]">Export as PNG first, then share</p>
                </div>
                ...
                <button onClick={shareWhatsApp}
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-[#AAA]">Share via WhatsApp</p>
                  </div>
                </button>
                ...
                <button onClick={shareEmail}
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-[#AAA]">Send via email</p>
                  </div>
                </button>
                ...
                <button onClick={shareNative}
                  ...
                  <div>
                    <p className="text-[12px] font-semibold text-[#333]">More options</p>
                    <p className="text-[10px] text-[#AAA]">System share sheet</p>
                  </div>
                </button>
              </div>
            )}
        ...
        <main className="flex-1 flex flex-col overflow-hidden bg-[#EBEBEB]">
          ...
          <div className="flex-1 flex items-center justify-center p-8 overflow-auto">
            <div>
              ...
              <p className="text-center text-[10px] text-[#AAA] mt-3">Certificate (Landscape) · 800 × 560</p>
            </div>
          </div>
        </main>
